...
/* 2 한 장 요약 */ {
  ...
  [['문제','약관은 서로 90%가 닮았다','섞어놓고 찾으면 아무거나 집어옵니다. 검색은 「모르겠다」를 못 해서 틀려도 티가 나지 않습니다.'],
   ['해법','찾기 전에 정확한 상품을 짚어낸다','보험사 → 상품 → 약관 시점으로 그 계약의 약관을 정확히 짚고, 그 안에서만 찾습니다.'],
...
/* 4 해법 */ {
  ...
  [['1단계','어느 약관인지 짚어낸다','AI가 추측하지 않습니다. 사용자가 고르거나 계약 정보로 정해집니다.'],
...
/* 6 확장 구조 */ {
  ...
  [['STEP 1','설계사 직접 선택','보험사 → 상품 → 약관 시점','약관 1권 확정','조항 조회 · 면책 확인','2~3번',C.deep,C.soft,'CFE9DB','1차 · 10/1'],
...
/* 비용 */ {
  ...
  s.addText('상품 하나가 파일 하나가 아닙니다. 보장형태 × 납입방법 × 개정으로 갈립니다.\n배수가 6이면 200건에 손보 상품이 4개만 들어갑니다.',
  ...
  s.addText('현재 판매 중인 약관 200개로 시작하고, 그 밖은 요청한 FA가 부담합니다.\n단가 미확정이라 지금은 금액을 띄우지 않고 요청 의사만 받습니다.',
  ...
  s.addText('개정분까지 세면 150건이 넘지만, 실손은 1~4세대 표준약관으로 갈리고 보상 판정의 갈래도 세대가 정합니다.\n세대 대표 약관만 사도 대부분의 질문을 덮습니다. 다만 갱신 · 전환으로 세대가 바뀌므로 「가입 시점」이 아니라 「지금 적용 중인 세대」를 물어야 합니다.',
...
/* 오픈 판정 + 지표 */ {
  ...
  s.addText('통과 기준 — 스코프 밖 인용 0건 · 면책 재현율 100% · 약관 시점 일치율 100% · 특약 오귀속 0건 · 1차 금액 언급 0건',
...
/* 남은 확인 */ {
  ...
  [['1','판매 종료 상품 색인 가능 여부','불가하면 보상용 약관을 살 수 없습니다. 기획 전제가 무너집니다','벤더',C.red],
   ['2','상품 · 약관 시점 필터 지원','스코프를 확정해도 벤더가 100권을 뒤지게 됩니다','벤더',C.red],
   ['3','약관 파일 단가와 과금 단위','건당 / 용량 / 정액 중 무엇인지에 따라 등록 전략이 달라집니다','벤더',C.amber],
   ['4','상품당 파일 수(배수)','배수 6이면 200건에 손보 상품이 4개만 들어갑니다','벤더',C.amber],
