...
/* 2 한 장 요약 */ {
  ...
  [['문제','약관은 서로 90%가 닮았다','섞어놓고 찾으면 아무거나 집어옵니다. 검색은 「모르겠다」를 못 해서 틀려도 티가 나지 않습니다.'],
   ['해법','찾기 전에 한 권으로 좁힌다','보험사 → 상품 → 가입 시점으로 약관 1권을 확정하고 그 안에서만 찾습니다.'],
...
/* 4 해법 */ {
  ...
  [['1단계','어느 약관인지 확정한다','AI가 추측하지 않습니다. 사용자가 고르거나 계약 정보로 정해집니다.'],
   ['2단계','그 약관 한 권만 펼친다','확정된 1권 안에서만 찾습니다. 밖의 조항은 점수가 높아도 버립니다.'],
...
function screen(eyebrow, title, sub, shot, notes) {
  const s = p.addSlide(); s.background={color:C.paper};
  head(s, eyebrow, title, sub);
  fit(s, shot, { x:0.6, y:1.72, w:8.7, h:5.0 });
  let y = 1.9;
  notes.forEach(([t,d])=>{
    card(s, { x:9.55, y, w:3.15, h:1.62 });
    s.addText(t, { x:9.78, y:y+0.18, w:2.7, h:0.44, fontFace:F.h, fontSize:12.5, bold:true, color:C.ink });
    s.addText(d, { x:9.78, y:y+0.64, w:2.7, h:0.84, fontFace:F.b, fontSize:10, color:C.body, lineSpacing:15 });
    y += 1.78;
  });
  foot(s);
}
...
screen('화면 ① 일반 답변', '먼저 일반으로 답하고, 개인화는 고르게 합니다',
  ...
  [['일반 답변은 실패가 아니다','매번 약관 등록을 요구하면 도구가 무거워집니다. 「이 답변으로 충분해요」를 같은 무게로 둡니다.'],
   ['입구는 약관이지 고객이 아니다','가입한 상품을 고르면 그 상품의 약관 기준으로 답이 바뀝니다. 고객 등록은 안 해도 됩니다.'],
   ['여기서 금액은 말하지 않는다','담보 구성 · 지급 조건 · 면책은 상품과 판마다 다르다는 유의를 함께 답니다.']]);

screen('화면 ② 약관 스코프 확정', '찾기 전에 약관 한 권으로 좁힙니다',
  '보험사는 치거나 눌러 고르고, 상품은 검색으로 매칭합니다. 판까지 정해져야 검색이 열립니다.', 'scope',
  [['보종은 묻지 않는다','한 단계를 더 두면 「장기인가 통합인가」를 멈춰서 생각하게 됩니다. 검색이 더 빠릅니다.'],
   ['이름이 겹치는 상품을 함께 본다','「참좋은동행」과 「동행Ⅱ」는 다른 약관입니다. 한 건만 자동 선택하지 않습니다.'],
   ['실손은 판이 아니라 세대로','1~4세대 표준약관이 보상 갈래를 정합니다. 판 150개를 늘어놓을 필요가 없습니다.']]);

screen('화면 ③ 답변', '면책을 먼저 판정하고 조항으로 근거를 답니다',
  '고객을 만들지 않고 그냥 묻는 경로입니다. 대부분의 상담이 여기서 끝납니다.', 'ask',
  [['면책이 먼저다','지급사유를 먼저 보이면 그 줄에서 읽기를 멈추고 아래 면책은 읽히지 않습니다.'],
   ['근거를 조항으로 제시한다','제5조 1항 6호까지 짚어야 FA가 고객에게 그대로 읽어줄 수 있습니다.'],
   ['어느 약관인지 항상 보인다','「참좋은운전자상해보험 2판」 — 안 보이면 틀려도 알아챌 수 없습니다.']]);

screen('화면 ④ 약관북', '고객 폴더를 열면 질문 범위가 그 고객으로 잠깁니다',
  '이 화면이 차별점의 실체입니다. 상담할수록 고객별 약관이 쌓입니다.', 'entry',
  [['폴더가 곧 검색 범위다','차승희 폴더에서 물으면 그 고객 약관 4건 안에서만 찾습니다. 남의 계약이 섞이지 않습니다.'],
   ['청구 누락을 잡는다','담보가 여러 계약에 걸치면 걸리는 계약을 모두 보여줍니다. 이게 폴더 질문의 가장 큰 효용입니다.'],
   ['답할 수 있는 수를 밝힌다','계약 4건이어도 약관이 2건뿐이면 「지금 답할 수 있는 건 2건」이라고 씁니다.']]);

screen('화면 ⑤ 약관 등록', '지금은 하나씩 넣습니다. My Data가 열리면 한 번에 들어옵니다',
  '경로를 빠른 순으로 놓되, 아직 안 열린 것은 두되 잠급니다.', 'register',
  [['안 열린 길도 보여준다','곧 열릴 경로가 있다는 사실이 안내이고, 「왜 하나씩 넣나」를 납득하는 근거가 됩니다.'],
   ['이미 말한 상품은 다시 안 찾는다','대화에서 말했다면 그것이 곧 등록 대상입니다. 지금 가장 빠른 길입니다.'],
   ['「최근 등록한 상품」은 뺐다','그 목록은 결국 다른 고객에게 등록했던 상품이라, 무관한 계약을 끌어옵니다.']]);
