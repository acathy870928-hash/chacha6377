...
## 3. 5단계 파이프라인 — 왜 청킹만으로는 부족한가

약관 질문에 제대로 답하려면 **원문 조각**이 아니라 **관계**가 필요합니다.

```
1차 OCR              스캔 → 텍스트                                   ocr.py
2차 문서 구조 인식     보통약관 / 특약 / 조 / 항 / 호 / 조항 성격        chunker.py + structure.py
3차 개념 정규화        "고액치료비암" = 질병 > 암 > 고액치료비암          concepts.py
4차 관계 생성          정의 ↔ 지급 ↔ 면책 ↔ 감액 조항 연결               concepts.py
5차 검색              "고액암 조건?" → 그 특약의 정확한 조항으로          search.py
```
...
### 특약 계층이 없으면 조 번호는 주소가 아니다

실제 약관은 보통약관 하나 + 특별약관 수십 개이고, **특약마다 제1조부터 다시 시작**합니다.

```
보통약관                        제5조(보험금의 지급사유)
고액치료비암진단담보특별약관       제5조(고액치료비암의 정의 및 진단확정)   ← 같은 "제5조"
상해입원일당특별약관             제5조(상해의 정의)                    ← 또 "제5조"
```

`structure.py` 가 특약 경계를 잡아 각 청크에 소속 특약과 **조항 성격**(정의/지급사유/면책/감액/
보험기간/계약/절차)을 찍습니다. 조항 성격은 제목을 먼저 보되, **본문에 증거가 있으면 제목보다
우선**합니다 — "보험금 지급에 관한 세부규정"이라는 제목이라도 본문이 "50%를 지급합니다"면
그건 감액 조항입니다.

```bash
$ python -m terms_rag chunk 약관.pdf
[4] 특별약관  고액치료비암진단담보특별약관  제5조(고액치료비암의 정의 및 진단확정)  역할=정의
[6] 특별약관  고액치료비암진단담보특별약관  제7조(보험금 지급에 관한 세부규정)      역할=감액
```

### 3차: 약관이 정의하지 않은 개념은 만들지 않는다

**"고액치료비암"을 봤다고 곧바로 "암진단비"로 바꾸면 안 됩니다.** 같은 단어라도 약관마다,
2009년판과 2024년판이 다릅니다. 그래서 `concepts.py` 는 **정의 조항에서 실제로 정의된 용어만**
개념으로 승격시키고, 승격시킬 때 반드시 근거(특약·조·페이지·청크)를 붙입니다.

분류(`질병 > 암`)도 **정의 본문을 근거로** 합니다 — 용어 이름만 보고 추측하지 않습니다.
"악성신생물", "한국표준질병사인분류" 같은 문구가 정의 안에 있어야 `질병 > 암`이 됩니다.

```bash
$ python -m terms_rag terms 약관.pdf
■ 고액치료비암  [보장대상 정의]
   표준 개념 : 질병 > 암 > 고액치료비암
   근거      : 고액치료비암진단담보특별약관 제5조(고액치료비암의 정의 및 진단확정) p.100
   정의      : 한국표준질병사인분류에 있어서 뇌 및 중추신경계통의 암 … 악성신생물을

■ 진단확정  [지급조건]
   근거      : 고액치료비암진단담보특별약관 제5조(…) p.100
```

### 4차: 관계 생성

개념 하나에 대해 **같은 특약 안에서** 정의·지급·면책·감액 조항을 모읍니다.

```bash
$ python -m terms_rag graph 약관.pdf --out graph.json
■ 고액치료비암
   정의     → 고액치료비암진단담보특별약관 제5조(고액치료비암의 정의 및 진단확정) p.100
   지급조건  → 고액치료비암진단담보특별약관 제1조(보험금의 지급사유) p.98
   면책조건  → 고액치료비암진단담보특별약관 제6조(보험금을 지급하지 않는 사유) p.101
   감액조건  → 고액치료비암진단담보특별약관 제7조(보험금 지급에 관한 세부규정) p.101
```

### 5차: 질의 → 개념 → 특약 라우팅

사용자는 "고액암"이라 쓰고 약관은 "고액치료비암"이라 씁니다. 이게 안 이어지면 영영 못 만납니다.
`route_query` 가 **약관이 정의한 용어에 한해** 줄임말을 잇습니다(첫 글자가 같고 글자가 순서대로
들어 있을 때). 임의로 동의어를 만들지는 않습니다.

```
질문: "2009년 현대해상 하이라이프퍼펙트 고액암 조건?"

라우팅 OFF                                    라우팅 ON (기본)
#1 …특약 제7조(세부규정)                       #1 …특약 제7조(세부규정)
#2 제1조(목적)                        →       #2 …특약 제6조(지급하지 않는 사유)
#3 제6조(지급하지 않는 사유)                   #3 …특약 제1조(보험금의 지급사유)
   ↑ 보통약관이 섞임                              ↑ 해당 특약 조항으로 모임
```

`--clause 고액치료비암` (특약 한정), `--role 정의` (조항 성격 한정), `--no-auto-concept` 로
제어합니다.

### 아직 규칙 기반입니다

3차 정규화는 **정의 문형 정규식 + 정의 본문 근거 분류**로 동작합니다. 표준용어 사전
(예: 금감원 표준약관 용어)이나 LLM 추출 단계를 붙이면 `standard_term` 을 회사·연도가 달라도
같은 개념으로 묶을 수 있습니다. 지금은 `standard_term == raw_term` 이며, **틀린 정규화보다
정규화 안 함이 낫다**는 원칙으로 두었습니다.

---

## 4. 문서 메타데이터 (보험사·상품·시행일)
...
src/terms_rag/
  ...
  structure.py   보통약관/특약 계층 + 조항 성격 분류      ← 2차
  concepts.py    정의 추출·개념 정규화·관계 생성·질의 라우팅  ← 3·4·5차
  ...
  cli.py         meta / terms / graph / ocr / chunk / ingest / search / ask / export / info
...
- **스캔 PDF 는 OCR 을 먼저 거쳐야 합니다** (5장 참고). 파이프라인이 감지해서 알려주고
